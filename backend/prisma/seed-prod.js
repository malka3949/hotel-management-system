...
// All seed room IDs — rooms NOT in this list are orphaned test data
const SEED_ROOM_IDS = [
...
async function main() {
  // Remove orphaned rooms (not in seed list) and their reservations — test/legacy data
  await prisma.reservation.deleteMany({ where: { roomId: { notIn: SEED_ROOM_IDS } } });
  await prisma.room.deleteMany({ where: { id: { notIn: SEED_ROOM_IDS } } });
  // Remove orphaned room types (no rooms reference them)
  await prisma.roomType.deleteMany({ where: { rooms: { none: {} }, id: { notIn: [
