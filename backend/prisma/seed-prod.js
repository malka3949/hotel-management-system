...
async function cleanOrphanedRooms() {
  // Find rooms not in our seed list
  ...
  // Find reservations linked to orphaned rooms
  ...
  if (orphanedResvIds.length > 0) {
    ...
    await prisma.$executeRawUnsafe(
      `DELETE FROM check_ins WHERE reservation_id = ANY($1::uuid[])`,
      orphanedResvIds,
    );
    await prisma.$executeRawUnsafe(
      `DELETE FROM check_outs WHERE reservation_id = ANY($1::uuid[])`,
      orphanedResvIds,
    );
    await prisma.$executeRawUnsafe(
      `DELETE FROM online_check_ins WHERE reservation_id = ANY($1::uuid[])`,
      orphanedResvIds,
    );
    await prisma.$executeRawUnsafe(
      `DELETE FROM guest_access_tokens WHERE reservation_id = ANY($1::uuid[])`,
      orphanedResvIds,
    );
    await prisma.$executeRawUnsafe(
      `DELETE FROM reservation_guests WHERE reservation_id = ANY($1::uuid[])`,
      orphanedResvIds,
    );
    // payments cascade: payment_attempts → payments → charges/refunds; invoices → line items
    await prisma.$executeRawUnsafe(
      `DELETE FROM refunds WHERE payment_id IN (SELECT id FROM payments WHERE reservation_id = ANY($1::uuid[]))`,
      orphanedResvIds,
    );
    await prisma.$executeRawUnsafe(
      `DELETE FROM charges WHERE payment_id IN (SELECT id FROM payments WHERE reservation_id = ANY($1::uuid[]))`,
      orphanedResvIds,
    );
    await prisma.$executeRawUnsafe(
      `DELETE FROM payment_attempts WHERE reservation_id = ANY($1::uuid[])`,
      orphanedResvIds,
    );
    await prisma.$executeRawUnsafe(
      `DELETE FROM payments WHERE reservation_id = ANY($1::uuid[])`,
      orphanedResvIds,
    );
    await prisma.$executeRawUnsafe(
      `DELETE FROM invoice_line_items WHERE invoice_id IN (SELECT id FROM invoices WHERE reservation_id = ANY($1::uuid[]))`,
      orphanedResvIds,
    );
    await prisma.$executeRawUnsafe(
      `DELETE FROM invoices WHERE reservation_id = ANY($1::uuid[])`,
      orphanedResvIds,
    );
    await prisma.$executeRawUnsafe(
      `DELETE FROM reservations WHERE id = ANY($1::uuid[])`,
      orphanedResvIds,
    );
  }
  ...
  // Remove orphaned room types (not in seed, no rooms)
...
async function main() {
  ...
  const branch = await prisma.branch.upsert({
    ...
    update: { coverPhoto: 'https://res.cloudinary.com/dvnadprsu/image/upload/v1784667008/hotel-management/seed/branch-tlv-center.jpg', description: 'מיקום מרכזי עם גישה נוחה לכל אטרקציות תל אביב. עיצוב עכשווי, חדרים מרווחים ושירות חם ואישי.', amenities: ['בריכת גג','ספא','חדר כושר','WiFi חינם','חניה','שירות חדרים 24/7','מסעדה','בר'], cancellationPolicy: 'ביטול חינם עד 48 שעות לפני ההגעה. ביטול מאוחר יותר — חיוב בגובה לילה אחד.' },
    create: { id: '00000000-0000-0000-0000-000000000001', name: 'מלון מרכז תל אביב', address: 'רחוב הרצל 1, תל אביב', phone: '03-1234567', email: 'tlv@hotel.co.il', contactPerson: 'שרה לוי', coverPhoto: 'https://res.cloudinary.com/dvnadprsu/image/upload/v1784667008/hotel-management/seed/branch-tlv-center.jpg', description: 'מיקום מרכזי עם גישה נוחה לכל אטרקציות תל אביב. עיצוב עכשווי, חדרים מרווחים ושירות חם ואישי.', amenities: ['בריכת גג','ספא','חדר כושר','WiFi חינם','חניה','שירות חדרים 24/7','מסעדה','בר'], cancellationPolicy: 'ביטול חינם עד 48 שעות לפני ההגעה. ביטול מאוחר יותר — חיוב בגובה לילה אחד.' },
  });
  const branchTlv = await prisma.branch.upsert({
    where: { id: '9049714c-9180-4b85-a7a2-c75dd18988c2' },
    update: { coverPhoto: 'https://res.cloudinary.com/dvnadprsu/image/upload/v1784667010/hotel-management/seed/branch-tlv.jpg', description: 'מלון בוטיק יוקרתי במרכז תל אביב. בריכת גג, ספא מלא ושירות קונסיירז׳ 24/7.', amenities: ['בריכת גג','ספא','חדר כושר','WiFi חינם','חניה','שירות חדרים 24/7','מסעדת שף','בר גג'], cancellationPolicy: 'ביטול חינם עד 48 שעות לפני ההגעה. ביטול מאוחר יותר — חיוב בגובה לילה אחד.' },
    create: { id: '9049714c-9180-4b85-a7a2-c75dd18988c2', name: 'מלון תל אביב', address: 'רחוב דיזנגוף 50, תל אביב', phone: '03-7654321', email: 'tlv@hotel.com', contactPerson: 'דני לוי', coverPhoto: 'https://res.cloudinary.com/dvnadprsu/image/upload/v1784667010/hotel-management/seed/branch-tlv.jpg', description: 'מלון בוטיק יוקרתי במרכז תל אביב. בריכת גג, ספא מלא ושירות קונסיירז׳ 24/7.', amenities: ['בריכת גג','ספא','חדר כושר','WiFi חינם','חניה','שירות חדרים 24/7','מסעדת שף','בר גג'], cancellationPolicy: 'ביטול חינם עד 48 שעות לפני ההגעה. ביטול מאוחר יותר — חיוב בגובה לילה אחד.' },
  });
  const branchJer = await prisma.branch.upsert({
    where: { id: 'a7688152-ff33-4e4f-8cc4-848500bdb800' },
    update: { coverPhoto: 'https://res.cloudinary.com/dvnadprsu/image/upload/v1784667012/hotel-management/seed/branch-jerusalem.jpg', description: 'מלון הדגל שלנו בלב העיר העתיקה. נוף מרהיב לחומות ירושלים.', amenities: ['נוף לחומות','ארוחת בוקר','WiFi חינם','חניה','שירות חדרים','טרקלין אורחים','סיורים מודרכים'], cancellationPolicy: 'ביטול חינם עד 72 שעות לפני ההגעה. ביטול מאוחר יותר — חיוב בגובה לילה אחד.' },
