...
const SEED_ROOM_IDS = [
  'f5fb3e41-b6fe-4169-9449-d5990cdcbfad','dec86aa5-eea5-4c22-b1dd-5e681a246a03',
  'e1aeecb1-8716-43c9-ac55-ffb3b1b0199d','a4fef1fe-b37d-4567-9024-8026b45eac65',
  'f1a18087-6008-4cc3-a4d1-5f034010ec48','347db1ff-8bd1-4d56-83f4-a8bd5f1ee624',
  'bb91cf8f-b238-43e4-91fa-1572faa5fd28','22b1fd91-ecef-4ac8-afa5-94183e9cfe4d',
  '6a9d3385-9909-4803-86a8-d93939ffabd3','7ab21260-eae4-4139-a0df-0b1bedf68b0e',
  'ff1fd852-ae3f-4cff-ac44-7d833e8a3259','9d5082cc-19b1-433a-b412-8b974bc000a9',
  '846f7cc2-6a20-42d6-aa33-a309a6ddca9f','d3cd4076-303d-4c60-827e-3e0d1192ac51',
  'cc9e3cb5-e527-437f-a9a8-f48f222e34b3','7e54cde7-b89e-4a97-acd9-e7c2a487475e',
  '2ccf145f-d9d8-4d40-852a-06e7342b3989','a5292b9a-277a-41d3-a5c8-734027b2461c',
  '38cc4b22-b02e-48d3-86d5-bac3c5799818','ee82eb00-5edd-4c65-9c37-80e2b1510333',
  'd0d63f24-360c-47ea-8319-0055c817893f','c4348c2c-7b4e-4538-b011-6e280ed4d3b0',
  'ead60f4b-d7bf-451b-86b8-6cd6e11e313e','c6395e4a-d043-4b1e-8e19-e068a40d05cb',
  'b8d9c388-e8d6-4302-ae64-4d2fd7345a68','c919b759-8d1a-464b-90d6-b36df0f739b8',
  'e5c8e870-814a-491b-82cd-c67d32762262','5b72c55d-e21d-41e4-8492-ab0eae55d65b',
  'e27969c6-1731-4f57-a43b-ab193cd70a53','24e1841e-c283-4b6a-8748-4bfa8c67bfb2',
  '419a089a-cc23-4047-9182-9b5b79eaf096','6a280fcd-4021-4a92-9fba-3286cbb1b7e0',
  '045edada-68fb-40ae-a139-7064171b36e1','88eec552-38bd-4203-a23f-d65390fc72fe',
  '02af1f99-a121-454a-873d-e5982fd1f154','06151f19-1f07-408d-afa8-9365e80af50a',
  'b60c6967-9a1b-45d7-8ddc-ac01a03588ab','d7df2a31-b2f1-4adf-a2eb-066b0a3d72b6',
  'ac58ea9c-67f5-468e-b34c-4d79a23fa28e','68ec5534-ef4f-49b5-a79e-a549a43acb37',
  '3b3386fd-f151-41bc-8245-ee364cc491c1','b27a0f4e-ae0d-49d4-84aa-31708462e5c6',
  'e05115b4-e556-4107-ae9a-23228e77e1e2','6555447d-8a85-40aa-aa42-e6aa82d6a5c0',
];

const SEED_RT_IDS = [
  'rt-center-01','rt-center-02','rt-center-03','rt-center-04',
  'dfdf88d3-2fc8-4252-b5e3-d1655c01021b','f7f93802-cc7a-4989-a962-53e8d089df04',
  'b90c1724-204a-4aec-9ed4-4a3cbf55a088','3fcacc1b-5296-4b28-94dd-7798e66450f1',
  'df499fc0-d341-47ca-85c6-bbbe19108a20',
  'c49b9945-a973-47eb-a1a5-86507985f9b7','33f9fc3e-8009-4497-b7cf-b33a66c5df96',
  '26af1730-f2a2-4005-bc3f-f6111a79ff3a','581b9ce5-d4f4-43a3-abb3-704a39dbf369',
];

async function cleanOrphanedRooms() {
  const orphanedRooms = await prisma.room.findMany({
    where: { id: { notIn: SEED_ROOM_IDS } },
    select: { id: true },
  });
  if (orphanedRooms.length === 0) return;
  const orphanedRoomIds = orphanedRooms.map(r => r.id);

  const orphanedResv = await prisma.reservation.findMany({
    where: { roomId: { in: orphanedRoomIds } },
    select: { id: true },
  });
  const orphanedResvIds = orphanedResv.map(r => r.id);

  if (orphanedResvIds.length > 0) {
    await prisma.checkIn.deleteMany({ where: { reservationId: { in: orphanedResvIds } } });
    await prisma.checkOut.deleteMany({ where: { reservationId: { in: orphanedResvIds } } });
...
async function main() {
  await cleanOrphanedRooms();

  // ── Branches ───────────────────────────────────────────────────────────────────────────────
  const branch = await prisma.branch.upsert({
    where: { id: '00000000-0000-0000-0000-000000000001' },
    update: { coverPhoto: 'https://res.cloudinary.com/dvnadprsu/image/upload/v1784667008/hotel-management/seed/branch-tlv-center.jpg', description: 'מיקום מרכזי עם גישה נוחה לכל אטרקציות תל אביב. עיצוב עכשווי, חדרים מרווחים ושירות חם ואישי.', amenities: ['בריכת גג','ספא','חדר כושר','WiFi חינם','חנייה','שירות חדרים 24/7','מסעדה','בר'], cancellationPolicy: 'ביטול חינם עד 48 שעות לפני ההגעה. ביטול מאוחר יותר — חיוב בגובה לילה אחד.' },
    create: { id: '00000000-0000-0000-0000-000000000001', name: 'מלון מרכז תל אביב', address: 'רחוב הרצל 1, תל אביב', phone: '03-1234567', email: 'tlv@hotel.co.il', contactPerson: 'שרה לוי', coverPhoto: 'https://res.cloudinary.com/dvnadprsu/image/upload/v1784667008/hotel-management/seed/branch-tlv-center.jpg', description: 'מיקום מרכזי עם גישה נוחה לכל אטרקציות תל אביב. עיצוב עכשווי, חדרים מרווחים ושירות חם ואישי.', amenities: ['בריכת גג','ספא','חדר כושר','WiFi חינם','חנייה','שירות חדרים 24/7','מסעדה','בר'], cancellationPolicy: 'ביטול חינם עד 48 שעות לפני ההגעה. ביטול מאוחר יותר — חיוב בגובה לילה אחד.' },
  });
  const branchTlv = await prisma.branch.upsert({
    where: { id: '9049714c-9180-4b85-a7a2-c75dd18988c2' },
    update: { coverPhoto: 'https://res.cloudinary.com/dvnadprsu/image/upload/v1784667010/hotel-management/seed/branch-tlv.jpg', description: 'מלון בוטיק יוקרתי במרכז תל אביב. בריכת גג, ספא מלא ושירות קונסיירז׳ 24/7.', amenities: ['בריכת גג','ספא','חדר כושר','WiFi חינם','חנייה','שירות חדרים 24/7','מסעדת שף','בר גג'], cancellationPolicy: 'ביטול חינם עד 48 שעות לפני ההגעה. ביטול מאוחר יותר — חיוב בגובה לילה אחד.' },
    create: { id: '9049714c-9180-4b85-a7a2-c75dd18988c2', name: 'מלון תל אביב', address: 'רחוב דיזנגוף 50, תל אביב', phone: '03-7654321', email: 'tlv@hotel.com', contactPerson: 'דני לוי', coverPhoto: 'https://res.cloudinary.com/dvnadprsu/image/upload/v1784667010/hotel-management/seed/branch-tlv.jpg', description: 'מלון בוטיק יוקרתי במרכז תל אביב. בריכת גג, ספא מלא ושירות קונסיירז׳ 24/7.', amenities: ['בריכת גג','ספא','חדר כושר','WiFi חינם','חנייה','שירות חדרים 24/7','מסעדת שף','בר גג'], cancellationPolicy: 'ביטול חינם עד 48 שעות לפני ההגעה. ביטול מאוחר יותר — חיוב בגובה לילה אחד.' },
  });
  const branchJer = await prisma.branch.upsert({
    where: { id: 'a7688152-ff33-4e4f-8cc4-848500bdb800' },
    update: { coverPhoto: 'https://res.cloudinary.com/dvnadprsu/image/upload/v1784667012/hotel-management/seed/branch-jerusalem.jpg', description: 'מלון הדגל שלנו בלב העיר העתיקה. נוף מרהיב לחומות ירושלים.', amenities: ['נוף לחומות','ארוחת בוקר','WiFi חינם','חנייה','שירות חדרים','טרקלין אורחים','סיורים מודרכים'], cancellationPolicy: 'ביטול חינם עד 72 שעות לפני ההגעה. ביטול מאוחר יותר — חיוב בגובה לילה אחד.' },
    create: { id: 'a7688152-ff33-4e4f-8cc4-848500bdb800', name: 'מלון ירושלים', address: 'רחוב יפו 10, ירושלים', phone: '02-1234567', email: 'jer@hotel.com', contactPerson: 'רחל כהן', coverPhoto: 'https://res.cloudinary.com/dvnadprsu/image/upload/v1784667012/hotel-management/seed/branch-jerusalem.jpg', description: 'מלון הדגל שלנו בלב העיר העתיקה. נוף מרהיב לחומות ירושלים.', amenities: ['נוף לחומות','ארוחת בוקר','WiFi חינם','חנייה','שירות חדרים','טרקלין אורחים','סיורים מודרכים'], cancellationPolicy: 'ביטול חינם עד 72 שעות לפני ההגעה. ביטול מאוחר יותר — חיוב בגובה לילה אחד.' },
  });
