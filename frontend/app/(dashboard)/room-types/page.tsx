...
function PhotosInput({ photos, onChange }: { photos: string[]; onChange: (p: string[]) => void }) {
  ...
  return (
    <div className="space-y-2">
      <p className="text-xs" style={{ color: 'var(--color-text-secondary)' }}>תמונות (עד 5)</p>
...
function AmenitiesInput({ amenities, onChange }: { amenities: string[]; onChange: (a: string[]) => void }) {
  ...
  return (
    <div className="space-y-2">
      <p className="text-xs" style={{ color: 'var(--color-text-secondary)' }}>שירותים (amenities)</p>
      <div className="flex gap-2">
        <input
          placeholder="WiFi, מרפסה, ג'קוזי..."
          value={input}
      ...
      <div className="flex flex-wrap gap-1">
        {amenities.map((a) => (
          <span key={a} className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-full" style={{ backgroundColor: 'rgba(196,162,83,0.12)', color: 'var(--color-accent)' }}>
            {a}
            <button type="button" onClick={() => onChange(amenities.filter((x) => x !== a))} className="hover:text-red-500">×</button>
          </span>
        ))}
      </div>
    </div>
...
export default function RoomTypesPage() {
  ...
  async function handleUpdate(id: string) {
    ...
    try {
      ...
      setRoomTypes((prev) => prev.map((rt) => (rt.id === updated.id ? updated : updated)));
      setEditingId(null);
    } catch (err) {
            ...
            {showCreate && (
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-3">
                  <input type="number" placeholder="מחיר בסיס (₪)" value={createForm.basePrice} onChange={(e) => setCreateForm((p) => ({ ...p, basePrice: e.target.value }))} required min={0} step="0.01" className="rounded-md border px-3 py-2 text-sm" style={{ borderColor: 'var(--color-border-default)' }} />
                  <input type="number" placeholder="קיבולת מקסימלית" value={createForm.maxOccupancy} onChange={(e) => setCreateForm((p) => ({ ...p, maxOccupancy: e.target.value }))} required min={1} className="rounded-md border px-3 py-2 text-sm" style={{ borderColor: 'var(--color-border-default)' }} />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <select value={createForm.bedType} onChange={(e) => setCreateForm((p) => ({ ...p, bedType: e.target.value }))} className="rounded-md border px-3 py-2 text-sm" style={{ borderColor: 'var(--color-border-default)' }}>
                    <option value="">סוג מיטה (אופציונלי)</option>
                    {['בודד', 'זוגית', 'טווין', 'קינג', 'שתי מיטות'].map((t) => <option key={t} value={t}>{t}</option>)}
                  </select>
                  <input type="number" placeholder="גודל החדר במ״ר" value={createForm.roomSize} onChange={(e) => setCreateForm((p) => ({ ...p, roomSize: e.target.value }))} min={1} className="rounded-md border px-3 py-2 text-sm" style={{ borderColor: 'var(--color-border-default)' }} />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <input type="number" placeholder="מקס׳ מבוגרים (אופציונלי)" value={createForm.maxAdults} onChange={(e) => setCreateForm((p) => ({ ...p, maxAdults: e.target.value }))} min={1} className="rounded-md border px-3 py-2 text-sm" style={{ borderColor: 'var(--color-border-default)' }} />
                  <input type="number" placeholder="מקס׳ ילדים (אופציונלי)" value={createForm.maxChildren} onChange={(e) => setCreateForm((p) => ({ ...p, maxChildren: e.target.value }))} min={0} className="rounded-md border px-3 py-2 text-sm" style={{ borderColor: 'var(--color-border-default)' }} />
                </div>
                <input placeholder="תיאור (אופציונלי)" value={createForm.description} onChange={(e) => setCreateForm((p) => ({ ...p, description: e.target.value }))} className="w-full rounded-md border px-3 py-2 text-sm" style={{ borderColor: 'var(--color-border-default)' }} />
                ...
                <div className="flex gap-2">
                  ...
                  <button type="button" onClick={() => setShowCreate(false)} className="text-sm px-4 py-2 rounded-md border" style={{ borderColor: 'var(--color-border-default)' }}>ביטול</button>
                </div>
              </form>
            )}
            ...
            {loading ? (
              ...
              <div className="rounded-lg border overflow-hidden" style={{ borderColor: 'var(--color-border-default)' }}>
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {roomTypes.map((rt, i) => (
                      <tr key={rt.id} style={{ borderTopColor: 'var(--color-border-default)', borderTopWidth: i > 0 ? 1 : 0, borderTopStyle: 'solid' }}>
                        {editingId === rt.id ? (
                          <>
                            ...
                            <td className="px-4 py-2"><input value={editForm.name} onChange={(e) => setEditForm((p) => ({ ...p, name: e.target.value }))} className="w-full rounded border px-2 py-1 text-sm" style={{ borderColor: 'var(--color-border-default)' }} /></td>
                            <td className="px-4 py-2"><input type="number" value={editForm.basePrice} onChange={(e) => setEditForm((p) => ({ ...p, basePrice: e.target.value }))} min={0} step="0.01" className="w-24 rounded border px-2 py-1 text-sm" style={{ borderColor: 'var(--color-border-default)' }} /></td>
                            <td className="px-4 py-2"><input type="number" value={editForm.maxOccupancy} onChange={(e) => setEditForm((p) => ({ ...p, maxOccupancy: e.target.value }))} min={1} className="w-16 rounded border px-2 py-1 text-sm" style={{ borderColor: 'var(--color-border-default)' }} /></td>
                            <td className="px-4 py-2" colSpan={2}>
                              <div className="grid grid-cols-2 gap-2 mb-2">
                                <select value={editForm.bedType} onChange={(e) => setEditForm((p) => ({ ...p, bedType: e.target.value }))} className="rounded border px-2 py-1 text-xs" style={{ borderColor: 'var(--color-border-default)' }}>
                                  <option value="">סוג מיטה</option>
                                  {['בודד', 'זוגית', 'טווין', 'קינג', 'שתי מיטות'].map((t) => <option key={t} value={t}>{t}</option>)}
                                </select>
                                <input type="number" placeholder="גודל מ״ר" value={editForm.roomSize} onChange={(e) => setEditForm((p) => ({ ...p, roomSize: e.target.value }))} min={1} className="rounded border px-2 py-1 text-xs" style={{ borderColor: 'var(--color-border-default)' }} />
                              </div>
                              <div className="grid grid-cols-2 gap-2 mb-2">
                                <input type="number" placeholder="מקס׳ מבוגרים" value={editForm.maxAdults} onChange={(e) => setEditForm((p) => ({ ...p, maxAdults: e.target.value }))} min={1} className="rounded border px-2 py-1 text-xs" style={{ borderColor: 'var(--color-border-default)' }} />
                                <input type="number" placeholder="מקס׳ ילדים" value={editForm.maxChildren} onChange={(e) => setEditForm((p) => ({ ...p, maxChildren: e.target.value }))} min={0} className="rounded border px-2 py-1 text-xs" style={{ borderColor: 'var(--color-border-default)' }} />
                              </div>
                              <input value={editForm.description} onChange={(e) => setEditForm((p) => ({ ...p, description: e.target.value }))} className="w-full rounded border px-2 py-1 text-sm mb-2" style={{ borderColor: 'var(--color-border-default)' }} />
                              <PhotosInput photos={editForm.photos} onChange={(photos) => setEditForm((p) => ({ ...p, photos }))} />
                              <div className="mt-2"><AmenitiesInput amenities={editForm.amenities} onChange={(amenities) => setEditForm((p) => ({ ...p, amenities }))} /></div>
                              <div className="flex gap-2 mt-2">
                                <button onClick={() => handleUpdate(rt.id)} disabled={saving} className="text-xs px-2 py-1 rounded text-white" style={{ backgroundColor: 'var(--color-primary)' }}>{saving ? '...' : 'שמור'}</button>
                                <button onClick={() => setEditingId(null)} className="text-xs px-2 py-1 rounded border" style={{ borderColor: 'var(--color-border-default)' }}>ביטול</button>
                              </div>
                            </td>
                          </>
                        ) : (
                          <>
                            ...
                            <td className="px-4 py-3 font-medium" style={{ color: 'var(--color-text-primary)' }}>{rt.name}</td>
                            <td className="px-4 py-3" style={{ color: 'var(--color-text-secondary)' }}>₪{Number(rt.basePrice).toLocaleString('he-IL')}</td>
                            <td className="px-4 py-3" style={{ color: 'var(--color-text-secondary)' }}>{rt.maxOccupancy}</td>
                            <td className="px-4 py-3" style={{ color: 'var(--color-text-secondary)' }}>{rt.description ?? '—'}</td>
                            <td className="px-4 py-3">
                              <div className="flex gap-2">
                                <button onClick={() => startEdit(rt)} className="text-xs px-2 py-1 rounded border" style={{ borderColor: 'var(--color-border-default)', color: 'var(--color-text-secondary)' }}>עריכה</button>
                                <button onClick={() => handleDelete(rt)} className="text-xs px-2 py-1 rounded border" style={{ borderColor: '#FCA5A5', color: '#DC2626' }}>מחיקה</button>
                              </div>
                            </td>
                          </>
                        )}
                ...
                {roomTypes.length === 0 && (
                  <p className="px-4 py-8 text-center text-sm" style={{ color: 'var(--color-text-secondary)' }}>אין סוגי חדרים</p>
                )}
              </div>
            )}
          </>
